...
import { PolygonItem } from "../icons/hexagonicons";

const Header = ({showBack,handlePolygonClick,}:{  handlePolygonClick: (polygon: PolygonItem | null) => void;
  showBack?: boolean;}) => {
  ...
  return (
    <nav className="bg-white shadow-sm border-b-2 border-gray-300 lg:border-0 rounded-xl flex justify-between items-center px-4 lg:px-[1%] py-2 lg:py-[0.8%] ">
      ...
      {showBack && (
  <button
    onClick={() => handlePolygonClick(null)}
  ...
  >
   <ArrowLeft  className="text-blue-300" /> Back
        ...
        {/* Desktop Menu */}
        <HeaderLinks  handlePolygonClick={handlePolygonClick} />
      </div>
