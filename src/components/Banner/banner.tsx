...
const { 
  ...
  polygonData.find(p => p.id === activePolygonId) || null;
const handlePolygonClick = (polygon: PolygonItem | null) => {
  ...
  if (activePolygonId === polygon.id) return;
          ...
          {polygonData.map((item, index) => (
            <Polygon
              ...
              className={clsx(
                ...
                item.position,
                item.size,
selectedPolygon?.id === item?.id                  ? "opacity-0 scale-0 pointer-events-none"
...
/>
                          ...
                          <BottomNav  handlePolygonClick={handlePolygonClick}/>
...
interface SelectedPolygonContentProps {
  selectedPolygon: PolygonItem;
  stage: string;
}

const SelectedPolygonContent = ({ selectedPolygon, stage }: SelectedPolygonContentProps) => (
  <div className={`relative -mt-8 lg:mt-[3vw] `}>
    <div
      className={`relative mx-auto flex items-center justify-center   
  size-48 lg:size-[13vw]`}
    >
      {stage === "loading" ? (
        <motion.img
          src={loaderGif.src}
